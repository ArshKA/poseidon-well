    ...
    def __getitem__(self, idx):
        ...
        try:
            with netCDF4.Dataset(self.data_file, 'r') as dataset:
                ...
                # Note: data is stored as (sample, time, x, y, [c]) -> we need (c, y, x)
                ...
                velocity_input = dataset.variables['velocity'][sample_idx, actual_t1, :, :, :].transpose(1, 0, 2)
                ...
                velocity_target = dataset.variables['velocity'][sample_idx, actual_t2, :, :, :].transpose(1, 0, 2)
        ...
        # Pad to square using zeros (already in normalized space)
    ...
    def get_ground_truth_for_rollout(self, idx, ar_steps_list):
        ...
        with netCDF4.Dataset(self.data_file, 'r') as nc_dataset:
            for step_size in ar_steps_list:
                ...
                # Note: data is stored as (sample, time, x, y, [c]) -> we need (c, y, x)
                buoyancy_target = nc_dataset.variables['buoyancy'][sample_idx, target_time, :, :].T
                pressure_target = nc_dataset.variables['pressure'][sample_idx, target_time, :, :].T
                velocity_target = nc_dataset.variables['velocity'][sample_idx, target_time, :, :, :].transpose(1, 0, 2)
