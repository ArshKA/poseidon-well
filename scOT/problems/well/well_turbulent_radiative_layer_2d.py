...
class WellTurbulentRadiativeLayer2D(BaseTimeDataset):
    """Well Turbulent Radiative Layer 2D dataset using assembled NetCDF file."""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        # Time constraint - we have 101 actual timesteps per trajectory (0 to 100)
        assert self.max_num_time_steps * self.time_step_size <= 101
        ...
        # Data specifications
        self.original_resolution = (128, 384)  # (height, width)
        h0, w0 = self.original_resolution
        self.resolution = max(h0, w0)  # INT side length of the square (e.g., 384)

        self.input_dim = 4  # density + pressure + velocity_x + velocity_y
        self.output_dim = 4  # Same as input
        self.label_description = "[density],[pressure],[velocity_x],[velocity_y]"
    ...
    def _find_assembled_file(self):
        ...
        if not os.path.exists(assembled_file):
            raise FileNotFoundError(
                ...
                f"Please run: python assemble_turbulent_radiative_layer_2D.py "
    ...
    def _load_normalization_constants(self):
        ...
        constants = {
            "time": 100.0,  # Time goes from 0 to 100
        }
    ...
    def __len__(self):
        ...
        # If there are 101 timesteps (0-100), the last input can be at t=100-time_step_size.
    ...
    def __getitem__(self, idx):
        ...
        try:
            with netCDF4.Dataset(self.data_file, 'r') as dataset:
                ...
                density_input = dataset.variables['density'][sample_idx, actual_t1, :, :]
                ...
                density_target = dataset.variables['density'][sample_idx, actual_t2, :, :]
                ...
                inputs = torch.cat([
                    torch.from_numpy(density_input.astype(np.float32)).unsqueeze(0),
                ...
                labels = torch.cat([
                    torch.from_numpy(density_target.astype(np.float32)).unsqueeze(0),
    ...
    def get_ground_truth_for_rollout(self, idx, ar_steps_list):
        ...
        with netCDF4.Dataset(self.data_file, 'r') as nc_dataset:
            for step_size in ar_steps_list:
                ...
                density_target = nc_dataset.variables['density'][sample_idx, target_time, :, :]
