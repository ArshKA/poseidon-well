#!/usr/bin/env python3
"""
Assemble Well Acoustic Scattering Maze dataset files into a single NetCDF file.
Based on the dataset configuration: 256x256 resolution, 202 timesteps.
Fields: pressure (scalar), velocity_x, velocity_y (vector components)
Constant fields: density, speed_of_sound (ignored in assembler)
"""
...
def assemble_acoustic_scattering_maze_data(input_dir, output_file):
    """Assemble all acoustic scattering maze files into a single NetCDF file."""
    ...
    with Dataset(nc_files[0], "r") as first_nc:
        ...
        height = pressure_var.shape[2]  # y dimension
        width = pressure_var.shape[3]   # x dimension
    ...
    with Dataset(output_file, "w") as out_nc:
        ...
        # Create variables for each field (ignoring constant fields like density, speed_of_sound)
        print("Creating variables...")
        
        # Scalar fields: pressure (sample, time, y, x)
        ...
        # Add metadata
        out_nc.setncattr("description", "Assembled Well Acoustic Scattering Maze dataset")
        ...
        out_nc.setncattr("fields", "pressure,velocity")
        out_nc.setncattr("dataset_name", "acoustic_scattering_maze")
    ...
    print(f"  - 2 fields: pressure, velocity(2) = 3 channels total")


def main():
    parser = argparse.ArgumentParser(description="Assemble Well Acoustic Scattering Maze dataset")
    parser.add_argument("--input_dir", type=str, required=True, 
                       help="Directory containing acoustic scattering maze .hdf5 files")
    ...
    assemble_acoustic_scattering_maze_data(args.input_dir, args.output_file)
